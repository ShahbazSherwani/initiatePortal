...
async function cleanupUser(email) {
  ...
  try {
    ...
    const userResult = await client.query(
      'SELECT firebase_uid, email, full_name, has_investor_account, has_borrower_account FROM users WHERE email = $1',
      [email]
    );
    ...
    if (userResult.rows.length === 0) {
      ...
      for (const row of userResult.rows) {
        console.log(`   - UID: ${row.firebase_uid}, Name: ${row.full_name}, Investor: ${row.has_investor_account}, Borrower: ${row.has_borrower_account}`);
      }
    }
    
    // Check for orphaned profiles by looking at email_verifications
    const verificationResult = await client.query(
      'SELECT firebase_uid, email FROM email_verifications WHERE email = $1',
      [email]
    );
    
    if (verificationResult.rows.length > 0) {
      console.log('📋 Found email_verifications:');
      for (const row of verificationResult.rows) {
        console.log(`   - UID: ${row.firebase_uid}`);
      }
    }
    ...
    // Also check investor_profiles and borrower_profiles for any with these UIDs
    for (const uid of allUids) {
      const investorCheck = await client.query('SELECT firebase_uid FROM investor_profiles WHERE firebase_uid = $1', [uid]);
      const borrowerCheck = await client.query('SELECT firebase_uid FROM borrower_profiles WHERE firebase_uid = $1', [uid]);
      
      if (investorCheck.rows.length > 0) {
        console.log(`📋 Found investor_profile for UID: ${uid}`);
      }
      if (borrowerCheck.rows.length > 0) {
        console.log(`📋 Found borrower_profile for UID: ${uid}`);
      }
    }
    ...
    for (const uid of allUids) {
      ...
      for (const { name, column } of tables) {
        try {
          ...
          console.log(`   ⚠️  Error deleting from ${name}: ${err.message}`);
        }
      }
    }
