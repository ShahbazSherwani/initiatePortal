  ...
  try {
    const result = await db.query(
      `INSERT INTO users (firebase_uid, full_name, role)
       ...
       RETURNING id, firebase_uid, full_name, email`,
...
app.get('/api/admin/investment-requests', verifyToken, async (req, res) => {
  ...
  try {
    ...
    for (const row of result.rows) {
      ...
      const pendingRequests = projectData.investorRequests?.filter(
        req => req.status === 'pending' || req.status === 'paid'
      ) || [];
      ...
      for (const request of pendingRequests) {
        ...
        pendingInvestments.push({
          ...
          date: request.date,
          status: request.status === 'paid' ? 'pending' : request.status,
