...
interface AccountContextType {
  ...
  setAccountType: (accountType: AccountType) => void; // Direct setter without validation
...
export const AccountProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('  - Has accounts:', { borrower: !!data.accounts?.borrower, investor: !!data.accounts?.investor });
        
        // If user has no accounts at all, treat them as new user and clear any stored account type
        if (!data.accounts?.borrower && !data.accounts?.investor) {
          console.log('  - User has no accounts, treating as new user');
          // Don't automatically default to borrower - respect localStorage or keep current state
          if (storedAccountType && (storedAccountType === 'borrower' || storedAccountType === 'investor')) {
            console.log('  - Using localStorage account type for new user:', storedAccountType);
            setCurrentAccountType(storedAccountType);
          } else {
            console.log('  - No stored account type, clearing localStorage');
            localStorage.removeItem('currentAccountType');
            // Don't force a default account type - let the user choose
          }
        } else if (data.user.currentAccountType) {
        ...
        // Remove the temporary fallback profile creation logic since we fixed the backend
        // The frontend should properly handle the case when user has no accounts
    ...
    try {
      ...
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('account-switched', { 
          detail: { accountType, timestamp: Date.now() } 
        }));
      }, 50);
  ...
  // Direct setter for account type without validation - useful after successful registration
  const setAccountType = (accountType: AccountType) => {
    console.log('🔧 Setting account type directly:', accountType);
    setCurrentAccountType(accountType);
    localStorage.setItem('currentAccountType', accountType);
  ...
  const createAccount = async (accountType: AccountType, data: any) => {
    ...
    try {
      ...
      // Important: Set the current account type immediately after creation
      setCurrentAccountType(accountType);
      localStorage.setItem('currentAccountType', accountType);
      
      console.log('✅ Account created and currentAccountType updated to:', accountType);
