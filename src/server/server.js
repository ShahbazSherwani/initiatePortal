      ...
      if (accountType === 'borrower') {
        ...
        await db.query(`
          INSERT INTO borrower_profiles (
            ...
            $75, $76, $77, $78, $79, $80, $81, $82, $83, $84, TRUE, NOW(), NOW()
          )
...
app.get('/api/user/qualified-investor/status', verifyToken, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT annual_income, gross_annual_income, is_qualified_investor, qi_request_status, qi_proof_url,
              qi_request_notes, qi_request_submitted_at, qi_granted_by, qi_granted_at
       FROM investor_profiles
       WHERE firebase_uid = $1`,
      [req.uid]
    );
  ...
  try {
    ...
    const result = await db.query(`
      SELECT u.firebase_uid, u.full_name, u.email,
             ...
             ip.is_qualified_investor, ip.qi_request_status, ip.qi_proof_url,
...
app.get('/api/projects/:id/escrow-status', verifyToken, async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    res.json({
      ...
      escrowUpdatedBy: projectData.escrowUpdatedBy || null
    });
  } catch (err) {
